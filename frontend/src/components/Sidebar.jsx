...
async function getConversationPreview(convo) {
  ...
  if (
    ...
    (convo.last_message.startsWith("audio:") ||
      convo.last_message.startsWith(AUDIO_PAYLOAD_PREFIX))
  ) {
  ...
  try {
    ...
    if (
      ...
      (decrypted.startsWith("audio:") ||
        decrypted.startsWith(AUDIO_PAYLOAD_PREFIX))
    ) {
      return "Voice message";
    }
...
export default function Sidebar({ activeConversationId, onSelect, onLogout }) {
  ...
  const filtered = convos.filter((c) => {
    const matchTab =
      tab === 0 ? true : tab === 1 ? c.unread_count > 0 : c.is_group;
    const q = searchFilter.trim().toLowerCase();
    const matchSearch =
      !q || (c.title || "Direct Chat").toLowerCase().includes(q);
      ...
      <div style={s.header}>
        ...
        <div style={s.headerRight}>
          ...
          <button
            style={s.iconBtn}
            title="New Group"
            onClick={() => setMode((m) => (m === "group" ? "list" : "group"))}
          >
            <GroupIcon />
          </button>
          <button
            style={s.iconBtn}
            title="New Chat"
            onClick={() => setMode((m) => (m === "search" ? "list" : "search"))}
          >
            <SearchIcon />
          </button>
          <button
      ...
      {mode === "list" && (
        <>
          ...
          <div style={s.filterWrap}>
            ...
            />
            {searchFilter && (
              <button style={s.clearBtn} onClick={() => setSearchFilter("")}>
                ✕
              </button>
            )}
          </div>
          ...
          <div style={s.tabs}>
            {TABS.map((t, i) => (
              ...
              >
                {t}
                {i === 1 && unreadTotal > 0 && (
                  <span style={s.tabBadge}>{unreadTotal}</span>
                )}
              </button>
            ))}
          </div>
            ...
            {filtered.map((c) => {
              ...
              return (
                <ConvoRow
                  ...
                  active={active}
                  onClick={() =>
                    onSelect(c.id, title, !!c.is_group, c.other_user_id ?? null)
                  }
                />
              );
            })}
      ...
      <div style={s.rowMid}>
        ...
        <div style={s.rowBot}>
          ...
          {convo.unread_count > 0 && (
            <span style={s.badge} className="badge-pulse">
              {convo.unread_count}
            </span>
          )}
        </div>
