...
async function tryDecrypt(msg, conversationId) {
  ...
  try {
    const decrypted = await decryptMessageWithFallback(msg.content, msg.iv, [
      msg.conversation_id,
      msg.conversationId,
      conversationId,
    ]);
...
/* ── Shared styles (module-level so MessageSkeleton can access s.bubble) ── */
const s = {
...
function InlineAudio({ src }) {
  ...
  useEffect(() => {
    ...
    function onEnded() { setPlaying(false); a.currentTime = 0; }
    ...
    <div style={s.list}>
      ...
      {messages.map((msg, i) => {
        ...
        return (
          ...
          >
            <div
              style={{ ...s.bubble, ...(isMine ? s.bubbleMe : s.bubbleThem) }}
            >
              {isMine && !msg.optimistic && (
                <div
                  ref={menuOpen === msg.id ? menuRef : null}
                  style={s.menuWrap}
                >
                  <button
                    style={s.menuBtn}
                    onClick={() =>
                      setMenuOpen((p) => (p === msg.id ? null : msg.id))
                    }
                  >
                    ⋮
                  </button>
